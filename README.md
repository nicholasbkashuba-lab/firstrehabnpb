...
## The AI assistant + intake

The bottom-right chat can now do two things: **answer visitor questions with AI**
(grounded only in this site's facts — services, hours, insurance, team, FAQ) and
**collect appointment requests**. The AI runs through `api/chat.js`, a Vercel
serverless function calling the Anthropic API — the `ANTHROPIC_API_KEY` lives in a
Vercel environment variable (Project → Settings → Environment Variables), never in
front-end code. Its knowledge base (`api/kb.json`) is regenerated by `build.py`, so
updating site content automatically updates the assistant. Guardrails: no medical
advice ever; clinical questions are redirected to booking an evaluation; off-topic
requests are declined; unknown answers defer to the front desk. If the API key
isn't configured, the assistant falls back gracefully to phone + front-desk messaging.
